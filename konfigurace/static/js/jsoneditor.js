...
function jsonEditorInit(table_container_id, json_input_container_id, json_output_container_id, json_to_table_btn_id, table_to_json_btn_id) {
    ...
    $.contextMenu({
        ...
        switch(key){
            ...
            case 'insertCR':
                ...
                $(`#json_table_${counter_id} tr[counter-id=${counter_id}]`).each(function(){
                    ...
                    new_th = `<th counter-id=${counter_id}><div contenteditable=false>`+ col_heading +'</div></th>'
        ...
        items: {
            "insertRD": {name: "Nový řádek pod buňkou", icon: "fas fa-trash-alt"},
            "insertRU": {name: "Nový řádek nad buňkou", icon: "fas fa-trash-alt"},
            "deleteR": {name: "Smazat řádek", icon: "fas fa-trash-alt"},
            "sep2": "---------",
            "copyRow": {name: "Kopírovat řádek", icon: "fas fa-arrows-alt-h"},
            "pasteRow": {name: "Vložit řádek", icon: "fas fa-arrows-alt-h"},
        }
    });
    ...
    table_template =`<table class = "json_table" counter-id="${max_counter}" id="json_table_${max_counter}">
                        <thead counter-id="${max_counter}" id="json_table_header_${max_counter}">
                            <tr counter-id="${max_counter}">
                                <th counter-id="${max_counter}"> <div contenteditable=false> id </div> </th>
                            </tr>
                        </thead>
    ...
    $.each(obj_for_table, function(level1_k, level1_v){
        $.each(level1_v, function(k, v){
            ...
            if(jQuery.type(v) == 'object'){
                ...
                if(v==''){
                    value = "<div contenteditable=true><select name=\"brand\" id=\"brands\"><option selected=>" + v + "</option><option value=\"tel\">tel</option><option value=\"native\">native</option><option value=\"browser\">browser</option></div>";
                    td_attr = "empty"
                }
                else if(v=='tel'){
                    value = "<div contenteditable=true><select name=\"brand\" id=\"brands\"><option selected>" + v + "</option><option value=\"empty\"></option><option value=\"native\">native</option><option value=\"browser\">browser</option></div>";
                    td_attr = "tel"
                }
                else if(v=='native'){
                    value = "<div contenteditable=true><select name=\"brand\" id=\"brands\"><option selected>" + v + "</option><option value=\"empty\"></option><option value=\"tel\">tel</option><option value=\"browser\">browser</option></div>";
                    td_attr = "native"
                }
                else if(v=='browser'){
                    value = "<div contenteditable=true><select name=\"brand\" id=\"brands\"><option selected>" + v + "</option><option value=\"empty\"></option><option value=\"tel\">tel</option><option value=\"native\">native</option></div>";
                    td_attr = "browser"
                }
                counter++;
                max_counter = counter;
...
function insertColumn(table_ref, header_name, counter) {
    ...
    $(inserted_td).replaceWith(`<th counter-id = ${counter}><div contenteditable=false>` + $(inserted_td).html() + `</div></th>`); //counter id at cell level
